...
def register_routes(app,settings,storage,jobs):
 ...
 async def comfyui_health():
  if not settings.comfyui_url: return {"configured":False,"available":False}
  from app.comfyui import ComfyUIClient,ComfyUIError
  try: await ComfyUIClient(settings.comfyui_url,settings.comfyui_timeout_seconds).health(); return {"configured":True,"available":True}
  except ComfyUIError as e: return {"configured":True,"available":False,"error":str(e)}
 @router.post("/jobs")
 async def create_job(character_image:UploadFile=File(...),reference_video:UploadFile=File(...),dialogue_audio:UploadFile=File(...)):
  ...
  try:
   ...
   storage.remove_job(jid); raise HTTPException(400,detail=str(e))
