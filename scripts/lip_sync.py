...
def main():
    ...
    if len(inputs) != 2:
        raise SystemExit(f"FAIL: expected 2 ONNX inputs, got {len(inputs)}")
    def is_mel(inp):
        shape = inp.shape
        if len(shape) != 4:
            return False
        h, w = shape[-2], shape[-1]
        try:
            return int(h) <= 80 and int(w) <= 16
        except (TypeError, ValueError):
            return "mel" in inp.name.lower() or "audio" in inp.name.lower() or "spec" in inp.name.lower()
    mel_inputs = [i for i in inputs if is_mel(i)]
    video_inputs = [i for i in inputs if not is_mel(i)]
    if len(mel_inputs) != 1 or len(video_inputs) != 1:
        raise SystemExit(f"FAIL: could not classify ONNX inputs: {[i.name for i in inputs]}")
    MEL_NAME = mel_inputs[0].name
    VIDEO_NAME = video_inputs[0].name
    print(f"Resolved mel input: {MEL_NAME}")
    print(f"Resolved video input: {VIDEO_NAME}")
    ...
    try:
        while frame_index < len(mel_chunks):
            ...
            if first_frame:
                ...
                writer = cv2.VideoWriter(silent_video, cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h))
                if not writer.isOpened():
                    raise SystemExit("FAIL: could not open silent output video writer")
                first_frame = False
            pred = model.run(None, {MEL_NAME: mels, VIDEO_NAME: images})[0]
    ...
    try:
        subprocess.run(["ffmpeg", "-y", "-i", silent_video, "-i", audio_path, "-c:v", "copy", "-c:a", "aac", "-shortest", output_path], check=True)
